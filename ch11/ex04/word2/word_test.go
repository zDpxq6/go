...
func randomPalindrome(rng *rand.Rand, ignore string) string {
	n := rng.Intn(10) // random length up to 24
	runes := make([]rune, n)
	for i := 0; i < (n+1)/2; i++ {
...
func randomNonPalindrome(rng *rand.Rand) string {
	...
	if IsPalindrome(result){
		result = randomNonPalindrome(rng)
	}
	return result
}
